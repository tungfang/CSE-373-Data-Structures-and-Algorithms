...
public class TestGraph extends BaseTest {
    public static class SimpleEdge<V> implements IEdge<V>, Comparable<SimpleEdge<V>> {
        private V vertex1;
        private V vertex2;
    ...
    public <V> void assertPathMatches(Graph<V, SimpleEdge<V>> graph, double expectedFinalCost, V[] expectedPath) {
    ...
    public void testFindingShortestPathSimple() {
        ...
        assertPathMatches(graph, 7, new String[] {"a", "d", "f"});
        assertPathMatches(graph, 7, new String[] {"f", "d", "a"});
        assertPathMatches(graph, 12, new String[] {"c", "a", "d", "f"});
        assertPathMatches(graph, 12, new String[] {"f", "d", "a", "c"});
    }
    ...
    public void testFindingShortestPathComplex() {
        ...
        assertPathMatches(graph, 6, new String[] {"a", "b", "c", "f", "e", "g"});
        assertPathMatches(graph, 6, new String[] {"g", "e", "f", "c", "b", "a"});
        assertPathMatches(graph, 6, new String[] {"g", "e", "f", "c", "d"});
        assertPathMatches(graph, 6, new String[] {"d", "c", "f", "e", "g"});
        assertPathMatches(graph, 1, new String[] {"a", "b"});
        assertPathMatches(graph, 1, new String[] {"b", "a"});
    }
    ...
    public void testFindingShortestPathDisconnectedComponents() {
        ...
        assertPathMatches(graph, 6, new String[] {"a", "b", "c", "f", "e", "g"});
        assertPathMatches(graph, 2, new String[] {"i", "k"});
    ...
    @Test(timeout=SECOND)
    public void testFindingShortestPathStartOrEndIsNull() {
        Graph<String, SimpleEdge<String>> graph = this.buildComplexGraph();
        try {
            graph.findShortestPathBetween(null, "a");
            fail("Expected IllegalArgumentException");
        } catch (IllegalArgumentException ex) {
        ...
        try {
            graph.findShortestPathBetween("a", null);
            fail("Expected IllegalArgumentException");
        } catch (IllegalArgumentException ex) {
            // Do nothing: this is ok
        }
